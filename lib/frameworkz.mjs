...
export async function startViewZ(options){
    ...
    if(options.container){
        ...
        // no container given, automatically add one to BODY
    ...
    if(options.cacheAllViews === false){
        ...
        shouldCacheAllViews = router.routes.length < 20 ;
    }
...
function addRoute(routes, routeToAdd, options){
    ...
    if(routeToAdd.subRoutes){
        ...
        for(let subRoute of routeToAdd.subRoutes){
            addRoute(routeOptions.routes, subRoute, options)
        }
    }
}