...
export async function startViewZ(options = "/viewz.config.json"){
    ...
    // automatically load stackerz-style.css (located next to this module)
    try{
        const _stackerzStyleUrl = new URL('./stackerz-style.css', import.meta.url).href;
        if(!document.querySelector(`link[href="${_stackerzStyleUrl}"]`)){
            const _link = document.createElement('link');
            _link.rel = 'stylesheet';
            _link.href = _stackerzStyleUrl;
            document.head.appendChild(_link);
        }
    }catch(e){
        // ignore if import.meta.url isn't available or URL creation fails
    }
