...
class RouteInstanceZ {
    ...
    async render({url, params, queries, hash, container, path, router, stacker}){
        let view = this.view ;
        view.route = {url, params, queries, hash, path} ;
        view.route.setParams = (params)=>{
            let newUrl = view.route.url ;
            for(let key of Object.keys(params)){
                //replace optionnal param
                newUrl = newUrl.replace(new RegExp("\{/:"+key+"\\}"), "/"+params[key]) ;
                //replace normal param
                newUrl = newUrl.replace(new RegExp("/:"+key), "/"+params[key]) ;
            }
            //remove remaining optionnal params
            newUrl = newUrl.replace(new RegExp("\{/:.*\\}"), "") ;
            router.navigateTo(
                router.getCurrentLocation().pathname.replace(router.base??"","")
                .replace(view.route.path.replace(router.base??"",""), newUrl)) ;
        }
        this.result = null;
        let ssr = false;
        ...
        window.dispatchEvent(new CustomEvent("viewOpen", {bubbles: true, detail : {view: this.view, route: {url, params, queries, hash}}})) ;
    }
}
class RouteZ {
...
export class RouterZ {
    ...
    */
    ...
    async cacheAllViews(syncLimit = 5){
        // "sync" first routes
        ...
        // then async load remaining routes (we don't await them)
        (async ()=>{
            for(let i=syncLimit; i<this.routes.length; i++){
                let route = this.routes[i] ;
                await route.view.prepareSources() ;
            }
        })() ;
    ...
    async _change(location) {
        this.changeRenderQueue.push(async ()=>{
            ...
            for(let i=0; i<pathAndStack.length; i++){
                ...
                if(this.currentStack){
                    ...
                    if(previousStackItem){
                        ...
                        if(previousPath === path){
                            ...
                            for(let k of Object.keys(newParams)){
                                routeInstance.view.route.params[k] = newParams[k] ;
                                viewedProperties.push(k);
                            }
                            for(let k of Object.keys(routeInstance.view.route.params)){
                                if(!viewedProperties.includes(k)){
                                    //this property is not in the new data, remove it
                                    delete routeInstance.view.route.params[k] ;
                                }
                            }

                            routeInstance.view.route.queries = data.queries;
                            routeInstance.view.route.hash = data.hash;
                            routeInstance.view.route.path = path;
                ...
                await this._openRoute({route, renderId, params: data.params, queries: data.queries, hash: data.hash, stacker, level: stackOptions?.level??0, path}) ;
    ...
    async _openRoute({route, renderId, params= {}, queries= {}, hash= "", stacker, openningParent = false, level=0, path}){
        ...
        if(route.parent){
            if(!stacker || !stacker?.inPlace){ // if there is a s//route.parent.url !== "/"){
                ...
                const subViewsContainers = route.parent.getInstance(level).view.querySelectorAll("z-sub-view") ;
                container = subViewsContainers.find(c=>c.getAttribute("level") == level) ;
                if(!container){
                    // no subview with the right level, use the first one
                    container = subViewsContainers[0] ;
                }
        ...
        if(container && !container.innerHTML.trim()){
            container.innerHTML = "&nbsp;" ; // force a content so viewz won't insert spinner
        }
        ...
        if((routeInstance.renderId||-1) < renderId){
            ...
            if(!routeInstance.renderId){
                ...
                await routeInstance.render({ url: route.url, params, queries, hash, container, path, router: this, stacker }) ;
