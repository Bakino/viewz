...
export class RouterZ {
    ...
    */
                ...
                (doc)=>{
                    ...
                    for(let a of allLinks){
                        if(a.hasAttribute("href") && a.getAttribute("href").startsWith("/")){
                            ...
                            a.setAttribute("z-on-click", `view.router.autoNavigate('${a.getAttribute("href")}', this.getAttribute("target"));`) ;
    ...
    async _openRoute({route, renderId, params= {}, queries= {}, hash= "", stacker, openningParent = false, level=0}){
        ...
        if(stacker){
            container = await stacker.getContainer({parentContainer: container}) ;
        }
        let routeInstance = route.getInstance(level) ;
        if(route.parent){
            if(!stacker || route.parent.url !== "/"){
                await this._openRoute({route: route.parent, renderId, stacker, openningParent: true, level}) ;
                container = route.parent.getInstance(level).view.querySelector("z-sub-view") ;
                if(!container){
        ...
        if(routeInstance.renderId !== renderId){
    ...
    async openStack({type, options=null, path}){
    ...
    async render(){
        ...
        try{
            ...
            if(this.view && this.renderedPath === path && this.viewController){
                ...
                this.dispatchEvent(new CustomEvent("viewOpen", {bubbles: true, detail : {view: this.view}})) ;
