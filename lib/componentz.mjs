...
export class ViewZElement extends HTMLElement {
    constructor(componentsDir, component) {
        ...
        if(this.innerHTML.trim()){
            ...
            this.params[component.name] = this.innerHTML.trim() ;
        }
        ...
        this.observer = new MutationObserver((mutationsList) => {
            ...
            for (const mutation of mutationsList) {
                if (mutation.type === 'attributes' && mutation.attributeName !== "style") {
                    ...
                    this.params[mutation.attributeName] = this.getAttribute(mutation.attributeName) ;
                }
            }
            if(hasModification){
                this.view.route.params = this.params;
    ...
    connectedCallback() {
        ...
        for (let i = 0; i < this.attributes.length; i++) {
            this.params[this.attributes[i].name] = this.attributes[i].value;
        }
...
export function createComponent(componentsDir, component){
    eval(`
        class ViewZComponent${component.name} extends ViewZElement {
            constructor(){
                super('${componentsDir}', ${JSON.stringify(component)})
            }
        }
        customElements.define('z-${component.name}', ViewZComponent${component.name});
    `) ;
}