...
class Waiter {
    ...
    setWaiterDefaultMessage(defaultMessage){
    ...
    showWaiter(){
        ...
        this.message = this.defaultMessage; //once shown forgot message to not display it again
    ...
    async wait(promise, {message}={}){
        ...
        try{
            ...
            warningTooLongId = setTimeout(()=>{
                ...
                console.warn("Waiter is still waiting after 10 seconds, maybe the promise is not resolved ?", promise) ;
            }, 10000) ;
            removeTooLongId = setTimeout(()=>{
                removeTooLongId = null;
                this.waitingCount-- ;
                if(this.waitingCount === 0){
                    //last waiter, hide it
                    this.hideWaiter()
                }
                console.warn("Waiter is still waiting after 60 seconds, force remove waiter", promise) ;
            }, 60000) ;
...
export function setWaiterErrorHandler(params){
    let parentElement = null;
    let handler;
    ...
    return waiterInstance.setErrorHandler(handler) ;
}

export function setWaiterDefaultMessage(params){
    let parentElement = null;
    let message;
    if(typeof(params) === "string"){
        message = params;
    ...
    return waiterInstance.setWaiterDefaultMessage(message) ;
}
