...
function extractPaths(expression) {
    ...
    for(let subSplit of closureSplit){
        ...
        // Extract paths from function arguments like my.function(a.variable, another.variable)
        const functionCallRegex = /\w+(?:\.\w+)*\(([^)]+)\)/g;
        let match;
        while ((match = functionCallRegex.exec(subSplit)) !== null) {
            const args = match[1];
            // Split arguments by comma, but be careful with nested functions
            const argPaths = args.split(',').map(arg => arg.trim());
            
            for (let arg of argPaths) {
                // Apply closure variable replacement if needed
                let processedArg = arg;
                if(closureVariable){
                    for(let [v, arrayName] of Object.entries(closureVariable)){
                        processedArg = processedArg.replaceAll(v+".", arrayName+".*.")
                    }
                }
                
                // Extract paths from the argument
                const argMatches = processedArg.replaceAll("?.", ".").match(regex) || [];
                allPaths = allPaths.concat(argMatches.filter(path => {
...
export class DataZ {
    ...
    applySpliceOnProxy(proxiedData, startIndex, deleteCount, addedItems, updatedSiblings = []) {
        if(startIndex < 0){ 
            console.warn("You try to splice on a negative index on ", proxiedData) ;
            return ; 
        }
    ...
    observe(data, dataPath = []) {
        ...
        const changeHandler = {
                            ...
                            for(let listener of listenersForThisPath.values()){
                                ...
                                if(mustRun){
                                    ...
                                    try{
                                        ...
                                        console.warn("Error while run listener ", listener, err) ;
                                        if(window.bindzErrorHandler || window.viewzErrorHandler){
                                            (window.bindzErrorHandler || window.viewzErrorHandler)(err, {
                                                type: "dataz-listener",
                                                listener, oldValue, newValue: value, target, property
                                            });
                                        }
                                    }
                                }
            ...
            get: (target, key) => {
                ...
                if(key === "indexOf" && Array.isArray(data)){
                    //provide a version of indexOf that search on underlying raw data
                    return function(objectToFind, fromIndex = 0){
                        if(objectToFind && objectToFind.__isProxy){
                            // trying to find a proxied object, we need to find its raw data
                            return data.findIndex((item,index)=>{
                                if(index<fromIndex){ return false ; }
                                if(item && item.__isProxy){
                                    return item.__rawData === objectToFind.__rawData ;
                                }
                                return item === objectToFind.__rawData ;
                            })
                        }
                        return Array.prototype.indexOf.call(data, objectToFind, fromIndex) ;
                    }
                }
                if(key === "lastIndexOf" && Array.isArray(data)){
                    //provide a version of indexOf that search on underlying raw data
                    return function(objectToFind, fromIndex = 0){
                        if(objectToFind && objectToFind.__isProxy){
                            // trying to find a proxied object, we need to find its raw data
                            ...
                            if(fromIndex !== undefined && !isNaN(fromIndex)){
                                ...
                                if(fromIndex >=0){
                                    startIndex = Math.min(startIndex, fromIndex);
                                }else{
                                    startIndex = data.length + fromIndex ;
                                }
                            }
                            for(let index = startIndex ; index >= 0 ; index--){
                                if(item && item.__isProxy){
                                    if(item.__rawData === objectToFind.__rawData ){
                                        return index ;
                                    }
                                }else if(item === objectToFind.__rawData){
                                    return index ;
                                }
                            }
                            return -1 ;
                        }
                        return Array.prototype.lastIndexOf.call(data, objectToFind, fromIndex) ;
                    }
                }
                if(key === "unshift"){
                    return function(){
...
export class BindZ {
     ...
     * @param {boolean} [isRootTree=true] - Indicate that this BindZ instance is an independant root tree or not
     */
    constructor(node, data = {}, context = {}, variablesPath= {}, isRootTree = true) {
        this.node = node;
        node.bindz = this;
    ...
    analyzeNode(node) {
        ...
        if(node.nodeType === Node.ELEMENT_NODE){
            ...
            // we are searching if the current node is a container of a subcomponent
            // a subcomponent can be a <z-sub-view> that contains other views or a <z-view> that contains an embeded view or an instance of a componentz (which also contains a view)
            const nodeIsComponentType = node.tagName === "Z-SUB-VIEW" || node.tagName === "Z-VIEW" || node.constructor?.name?.includes("ViewZComponent")
            const nodeIsRoot = (this.node === node) // this node is the root of the bindz
            const nodeIsRootTree = nodeIsRoot && this.isRootTree // this node is the root of the tree of the bindz
            
            let mustHandleBindAttribute = true ;
            let mustHandleHideAttribute = true ;
            let mustTraverse = true ;
            if(nodeIsComponentType){
                if(nodeIsRootTree){
                    // here the node is a component, and it is the root of the tree so we are analysing the component itself
                    // <div>
                    //    <z-comp> <-- root is here and we are analysing z-comp
                    // </div>
                    // we should not handle the bind attribute of this element because it belong to the parent context
                    mustHandleBindAttribute = false;
                    // we should not handle the hide attribute of this element because it belong to the parent context
                    mustHandleHideAttribute = false;
                    // we should traverse it because it is the component and we want to analyze its content
                    mustTraverse = true ;
                }else if(nodeIsRoot){
                    // it is not the root tree se we are not analyzing the component itself but a parent context
                    // <div>
                    //    <z-comp z-hide-if> <-- root is here but we are analysing z-hide-if
                    // </div>

                    // we handle the hide and bind attribute
                    mustHandleHideAttribute = true;
                    mustHandleBindAttribute = true;
                    // we don't traverse the node, its content belong to the component
                    mustTraverse = false ;
                }else{
                    // it is not the root tree se we are not analyzing the component itself but a parent context
                    // <div> <-- root is here
                ...
                // it is a normal node, analyze and traverse it
            ...
            if( !isRepeater && mustHandleHideAttribute && 
            ...
            } else if(mustHandleBindAttribute && node.hasAttribute("z-bind")){
                if(isRepeater){
                    //this is a repeater
                    const repeaterId = "repeater-"+idInc++;
            ...
            if(mustHandleBindAttribute){
                ...
                for(let a = 0; a < attributes.length; a++){
                    ...
                    if(!isSubComponent){ // if this node is the node of a subcomponent, the event are handled by parent, don't register them here
            ...
            return false ; //no traversal of text node
        }
    ...
    runExpression(expression, bindObj=null, isStringExpression = false, localContext = {}) {
        ...
        try{
            ...
            console.warn("Error when run function "+expression+" with args ",argsKeys,argsValues, err);
            // A ReferenceError means the expression references a variable that is
            // not in the data scope (typically the view's loader failed, so the
            // expected data keys are missing). Visibility expressions already
            // tolerate this ("assume hiding"); align value expressions with that
            // behaviour and don't report it as an error, to avoid noise
            // during transient broken renders. Genuine runtime errors (e.g.
            // TypeError in an expression) are still reported.
            if((window.bindzErrorHandler || window.viewzErrorHandler) && err?.name !== "ReferenceError"){
                (window.bindzErrorHandler || window.viewzErrorHandler)(err, {
                    type: "bindz-run-expression",
                    expression, bindObj, isStringExpression, localContext, argsKeys, argsValues
                });
            }
            //throw err;
            return null;
    ...
    bindValue(binding) {
        ...
        if(att === "_value"){
            ...
            if(binding.hasOwnProperty("lastValue") && binding.lastValue === valueToBind){ return; }
                    ...
                    if(valueToBind === false){
                        ...
                        try{
                            ...
                            if(valueStr.length > 2000){
                                ...
                                elementToBind.setAttribute(att, valueStr);
                            }
                        }catch(err){
                            elementToBind.setAttribute(att, `Value not serializable, get value on element.${propertyName} property`);
                        }
    ...
    prepareBinding(binding){
        ...
        if(binding.bindAtt.startsWith("${view")) {
...
export class Hider {
    ...
    constructor(node, condition, data, placeholder, context, variablesPath) {
        ...
        this.bindz = new BindZ(node, data, context, variablesPath, false) ;
        this.listeners = [] ;
        this.hidden = null;
        this.prepared = false;
        this.started = false;
    ...
    updateVisibility() {
        ...
        if(shouldHide !== this.hidden){
            ...
            if(this.hidden){
                //hide the node
                if(this.node.parentElement || this.node.getRootNode()?.constructor === DocumentFragment){
                    //the node has a parent or is in a document fragment (the later happend if it is a repeater main element)
...
export class Repeater {
    ...
    prepare() {
        ...
        this.prepared = true ;
        if(!this.recurseProperty){
            this.preparedBindz = new BindZ(this.node, this.data, {}, false) ;
    ...
    createBinder(item, index, array){
        ...
        const bindz = new BindZ(cloneNode, this.data, {...this.context, ...context}, {...this.variablesPath, ...variablesPath}, false) ;
        if(this.recurseProperty){
