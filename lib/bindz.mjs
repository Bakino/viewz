...
function extractPaths(expression) {
    ...
    const paths = expression.replaceAll("?.", ".").match(regex) || [];
        ...
        if(value === undefined){
            ...
            for(let part of pathParts){
                if(data && data.constructor === Object){
                    ...
                    //if the data is an array, try to get the index
...
export class BindZ {
    ...
    analyzeNode(node) {
        ...
        if(node.nodeType === Node.ELEMENT_NODE){
            //analyze element
            if(node.hasAttribute("z-hide-if") || node.hasAttribute("z-show-if")){
    ...
    getValueFromPath(path){
        ...
        return this.data.getValueFromPath(this.substituteVariables(path));
    }
        ...
        if(binding.bindProp === "_value"){
            ...
            for(let event of events){
                binding.node.addEventListener(event, ()=>{
                    ...
                    const parent = this.getValueFromPath(parentPath) ;
                    if(parent){
