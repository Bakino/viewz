...
import { loadScript, waiter, setWaiterMessage, uuidv4, setWaiterErrorHandler, setWaiterDefaultMessage } from "./utilz.mjs";
...
export class ViewZ {
    ...
    static loadExtension(extension){
        ...
        if(extension.errorHandler){
            setWaiterErrorHandler(extension.errorHandler);
        }
        if(extension.defaultWaiterMessage){
            setWaiterDefaultMessage(extension.defaultWaiterMessage);
        }
    }
    ...
    constructor({html, js, css, id, spinner, htmlProcessors, context}){
...
//# sourceURL=${fileName}` ;
    ...
    async bind(){
        ...
        this.bindz = binding.bind(this.container, data, this.context) ;
    ...
    async render({container, ssr}){
        ...
        this.container.setAttribute("z-view-container", this.id) ;
        this.container.contextz = this.context ;
        ...
        if(!ssr && (!this.sourcesPrepared() || this.loader)){
            ...
            if(!container.innerHTML.trim()){
                // add only the spinner if the container is empty, otherwise we can assume that their is already a loading (from routerz for example)
                container.innerHTML = this.spinner;
            }
        }else{
            // add waiter to display spinner while viewz is loading
            // don't await it because we must run until the end of this function to display the view, 
            // adding await will provoke it to never finish
        ...
        if(this.loader){
            try{
                ...
                console.warn("Error while fetch data", err) ;
       ...
       /*if(this.container === container && this.container.getAttribute("z-view") === this.options.page){
        ...
        if(this.displayed){
            try{
                ...
                console.warn("Error in displayed function", err) ;
                if(window.bindzErrorHandler || window.viewzErrorHandler){
                    (window.bindzErrorHandler || window.viewzErrorHandler)(err, {
                        type: "viewz-displayed",
                        viewId: this.viewId,
                    });
                }
            }
        ...
        setTimeout(()=>{
            //change in timeout to allow animation on opacity
            this.container.style.opacity = 1;
            const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent);
            const isSafari = /^((?!chrome|android).)*safari/i.test(navigator.userAgent);
            if(isIOS || isSafari){
                // on ios/safari, we must force a reflow, if not some random part of the screen are not correctly displayed
